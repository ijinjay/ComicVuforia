...
namespace {
    ...
    NSString * SAMPLE_APPLICATION_ERROR_DOMAIN = @"vuforia_application";
}

@interface QCARSession ()
...
- (bool)startCamera:(QCAR::CameraDevice::CAMERA)camera viewWidth:(float)viewWidth andHeight:(float)viewHeight error:(NSError **)error {
    ...
    _projectionMatrix = QCAR::Tool::getProjectionGL(cameraCalibration, 2.0f, 2000.0f);
    return YES;
}
